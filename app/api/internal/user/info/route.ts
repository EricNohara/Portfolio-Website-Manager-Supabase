...
export async function GET(_req: NextRequest): Promise<NextResponse> {
    try {
        ...
        const { data: userData, error: userDataError } = await supabase
            .from("users")
            .select("name, phone_number, email, github_url, linkedin_url, portrait_url, resume_url, transcript_url, instagram_url, facebook_url, x_url, bio, current_position, current_company, current_address")
            .eq("id", user.id)
            .single();
        ...
        const { data: userProject, error: userProjectError } = await supabase
            .from("projects")
            .select("name, date_start, date_end, languages_used, frameworks_used, technologies_used, description, github_url, demo_url, thumbnail_url")
            .eq("user_id", user.id);
        ...
        const { data: publicApiLogs, error: publicApiLogsError } = await supabase
            ...
            .select("key_description, user_agent, requested_at, responded_at, status_code")
            .eq("user_id", user.id);
